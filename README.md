...
## 常年化（2026-07-20）
评分标准不写死在代码里：「题目与评分标准」页管理**作业模板**
（题目要求 + 评分维度 + 准入检查）。换年级/换题目 → 粘贴新评分表
→ DeepSeek 解析 → 老师确认 → 保存模板 → 建任务时选择。
任务保存模板【快照】，此后改模板不影响历史批改记录。
每个维度带判定方式：转写稿判 / 口语类(AI仅参考) / 画面判 / 老师手评。

## 流程（零本地安装）
学生视频传谷歌课室 → 老师批量下载 → **直接上传本平台**（单次 ≤1GB，
可分批，互不覆盖）→ 服务器抽音频/关键帧/转写（**提取完立刻删除视频**）
→ 队列批改（DeepSeek 判文本三维度，GLM-4V 判拍摄维度）→
复核（异常标黄）→ 下载总包（每生PDF + 班级Excel + 讲评PPT）→
谷歌课室逐个退还。
...
- 引擎 video_engine.py: video-2.0-20260720（常年化）
